...
# 수정: 환경변수에서 RAINDROP_TOKEN, GSHEET_ID 로드 제거 (시트에서 불러옵니다)
...
# ── 구글 시트에서 GSHEET_ID 및 RAINDROP_TOKEN 로드 ──────────────────────────────────
config_spreadsheet = gclient.open_by_key(
...
def get_raindrop_prompt_by_tag(site_category, tag):
    ...
    for row in cached_prompt_rows[1:]:
        ...
        site_cat  = row[2].strip()           # C열: 이미지태그
        tag_val   = row[3].strip()           # D열: 구분태그
